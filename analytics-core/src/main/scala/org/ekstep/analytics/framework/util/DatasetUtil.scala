...
class DatasetExt(df: Dataset[Row]) {
  ...
  def saveToBlobStore(storageConfig: StorageConfig, format: String, reportId: String, options: Option[Map[String, String]], partitioningColumns: Option[Seq[String]]): List[String] = {
    ...
    val files = if(dims.nonEmpty) {
      ...
      df.cache().repartition(1).write.format(format).options(opts).partitionBy(dims: _*).save(filePrefix + tempDir);
      map.foreach(f => {
      ...
      recordTime(df.cache().repartition(1).write.format(format).options(opts).save(filePrefix + tempDir), s"File write time taken -");
